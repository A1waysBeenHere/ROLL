"""
RLVR Pipeline with TransferQueue Integration

This module integrates TransferQueue into the RLVR (Reinforcement Learning from Verifiable Rewards) pipeline.
The integration uses the tq_pipeline_utils module for simplified data handling.

1. Data Flow Pattern:
   - Main controller gets data from remote workers (via Ray RPC)
   - Main controller writes data to TransferQueue using dataproto_to_kv_batch_meta()
   - Remote workers can read data from TransferQueue using kv_batch_meta_to_dataproto()

2. Integration Points:
   - Initialization: TransferQueue is initialized via init_tq() when transfer_queue.enable=True
   - Data Write: dataproto_to_kv_batch_meta() converts DataProto -> KVBatchMeta
   - Data Read: kv_batch_meta_to_dataproto() converts KVBatchMeta -> DataProto
   - Cleanup: TransferQueue is properly closed in the run method

3. Key Features:
   - Conditional enable: TQ is only initialized when transfer_queue.enable=True in config
   - Partition-based organization: Each training step has its own partition
   - KVBatchMeta handle: Data is referenced by KVBatchMeta instead of raw keys
   - Metadata preservation: Both tensor and non-tensor data are stored
   - Error handling: Graceful degradation if TransferQueue operations fail

4. Usage Example for Remote Workers:
   ```python
   from roll.utils.tq_pipeline_utils import kv_batch_meta_to_dataproto
   
   # In a remote worker, receive KVBatchMeta from pipeline
   kv_meta = batch.meta_info["tq_kv_meta"]
   
   # Read data from TransferQueue
   data_proto = kv_batch_meta_to_dataproto(kv_meta)
   
   # Or read specific fields only
   partial_data = kv_batch_meta_to_dataproto(kv_meta, fields=["log_probs", "attention_mask"])
   ```

5. Benefits:
   - Simplified API: Direct use of utils functions, no wrapper methods needed
   - Type safety: KVBatchMeta provides structured data handle
   - Decouples data producers from consumers
   - Enables asynchronous data access
"""
...
from roll.utils.tq_pipeline_utils import (
    init_tq,
    dataproto_to_kv_batch_meta,
    kv_batch_meta_to_dataproto,
    merge_kv_batch_metas,
    kv_batch_meta_put_fields,
    fmt_kv_meta,
)
...
class RLVRPipeline(BasePipeline):
    ...
    def __init__(self, pipeline_config: RLVRConfig):
        ...
        self.tq_enabled = getattr(self.pipeline_config, 'transfer_queue', None) is not None \
            and self.pipeline_config.transfer_queue.enable
        if self.tq_enabled:
            print("Initializing TransferQueue...")
            from roll.utils.tq_pipeline_utils import init_tq
            init_tq(self.pipeline_config.transfer_queue)
            print(f"[TQ] RLVRPipeline.__init__: TQ enabled, initialized in pipeline driver process")
            ...
            with tps_timer, Timer(name="step_total", logger=None) as step_total_timer:
                ...
                if self.tq_enabled:
                    ...
                    try:
                        ...
                        tags = [
                            {"global_step": global_step, "domain": batch.non_tensor_batch.get("domain", ["unknown"] * batch_size)[i]}
                            for i in range(batch_size)
                        ]
                        kv_meta = dataproto_to_kv_batch_meta(
                            data=batch,
                            partition_id=partition_id,
                            key_prefix=f"{global_step}_",
                            tags=tags,
                        )
                        batch.meta_info["tq_kv_meta"] = kv_meta
                        logger.info(f"Generated data written to TransferQueue: partition={partition_id}, "
                                    f"keys={kv_meta.keys[:5]}...")
                    except Exception as e:
                        logger.warning(f"Failed to write generated data to TransferQueue: {e}. Continuing without TQ storage.")
                ...
                with Timer(name="cal_ref_log_probs", logger=None) as cal_ref_log_probs_timer:
                    if self.pipeline_config.enable_reference:
                        ...
                        print("====> Compute_log_probs", flush=True)
                        ...
                        if not self.use_ref_model:
                            ...
                            ref_log_probs = self.reference.compute_log_probs(kv_meta, blocking=True)
                        print(f"====>{type(ref_log_probs)}", flush=True)
